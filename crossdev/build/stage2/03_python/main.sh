...
install_python() {
  ...
  local cflag="$3"
  env PYTHON_CONFIGURE_OPTS="$configure_opts" PYTHON_CFLAGS="$cflag" pyenv install -v "$v"
}
...
for version in $versions; do
  ...
  configure_opts='--enable-optimizations --enable-option-checking=fatal --with-lto --with-ensurepip --enable-shared'
  ...
  if version_compare "$version" '>=' '3.11'; then
    configure_opts="$configure_opts --enable-pystats"
  fi
  ...
  if version_compare "$version" '<' '3'; then
    ...
    LDFLAGS="-L$openssl_backport_path/lib" CPPFLAGS="-I$openssl_backport_path/include" install_python "$version" "$configure_opts" "$cflag -Wl,-rpath=$openssl_backport_path/lib"
